<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Curriculums</h1>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="curriculum-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/curriculums/new"
          data-automation-id="curriculum-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(curriculums ?? []) as unknown as Curriculum[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToCurriculum"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="curriculum-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load curriculums: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Curriculums List Page - Showcase of mentorhub_spa_utils ease of use
 * 
...
const {
  ...
  setSortBy,
  setOrder,
} = useInfiniteScroll<Curriculum>({
  queryKey: ['curriculums'],
  queryFn: (params) => api.getCurriculums(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToCurriculum(_event: unknown, { item }: { item: Curriculum }) {
  router.push(`/curriculums/${item._id}`)
}
