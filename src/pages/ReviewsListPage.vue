<template>
  <v-container>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <ListPageSearch
          ...
          automation-id="review-list-search"
        />
      </v-col>
      <v-col cols="12" md="6" class="d-flex justify-end align-center">
        <v-btn 
          color="primary" 
          to="/reviews/new"
          data-automation-id="review-list-new-button"
        >
    ...
    <v-row>
      <v-col>
        <v-card>
          <v-data-table
            :headers="headers"
            :items="(reviews ?? []) as unknown as Review[]"
            :loading="isLoading as unknown as boolean"
            @click:row="navigateToReview"
            hover
            :items-per-page="-1"
            hide-default-footer
          >
          ...
          <v-card-actions v-if="hasMoreValue">
            <v-btn
              ...
              color="primary"
              block
              data-automation-id="review-list-load-more"
            >
    ...
    <v-snackbar :model-value="showError as unknown as boolean" color="error" :timeout="5000">
      Failed to load reviews: {{ errorMessage }}
    </v-snackbar>
  </v-container>
</template>
...
/**
 * Reviews List Page - Showcase of mentorhub_spa_utils ease of use
 * 
...
const {
  ...
  queryKey: ['reviews'],
  queryFn: (params) => api.getReviews(params),
  getItemId: (item) => item._id,
  limit: 20,
})

// Simple navigation handler
function navigateToReview(_event: unknown, { item }: { item: Review }) {
